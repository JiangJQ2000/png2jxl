...
def _palette_golden() -> tuple[bytes, bytes]:
    fixture = json.loads((FIXTURES / "v1-palette.json").read_text())
    return (
...
def test_v1_palette_golden_archive_reconstructs_exact_source() -> None:
    source, archive = _palette_golden()
    assert is_png_reconstructable_jxl(archive)
    assert jxl_to_png(archive) == source


def test_v1_palette_golden_archive_is_an_ordinary_jxl() -> None:
